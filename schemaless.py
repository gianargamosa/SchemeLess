...
class Database:
    ...
    def get():
        if Database.db is None:
            Database.db = torndb.Connection(host='localhost' ,  database='bar',
                                            user='root', password='th3sad0n3', connect_timeout=5)
        return Database.db
...
class Index():
    ...
    def __init__(self, indexOrindexes, indexTable, entitiyFieldInIndexTable):
        self.indexTable = indexTable
        self.indexOrindexes = indexOrindexes
        if isinstance(indexOrindexes, str):
            self.indexOrindexes = []
            self.indexOrindexes.append(indexOrindexes)
    ...
    def _determeinFieldsAndValues(self, model):
        _fields = []
        values = []
        for field in self.indexOrindexes:
...
class Fql():
    ...
    def _determineIndexValue(self, index):
        intersect = set(self.paramList).intersection(index.indexOrindexes)
        if len(self.paramList) == 0:
            return 0
        return float(len(intersect)) / float(len(self.paramList))

    def determineIndex(self, indexes):
        maxValue = 0
        selectIndex = None
...
class Model():
    ...
    def fqlGet(cls, query, *args):
        fql = Fql(query)
        index = fql.determineIndex(cls.indexes)
...
class Cleaner():
    ...
    def cleanModel(self, model, indexes = None, dttm = None):
